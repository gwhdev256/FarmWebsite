...
from models.table_data import TableDataModel
...
class CreateTable(Resource):
    ...
    def post(self):
        ...
        if TableDataModel.find_by_table_name(data["table_name"]):
            return {"message": "A table with that name already exists."}, 404

        table = TableDataModel(data["table_name"], data["table_headers"], data["table_data"])
...
class Table(Resource):
    ...
    def get(self, tablename):
        table = TableDataModel.find_by_table_name(tablename)
